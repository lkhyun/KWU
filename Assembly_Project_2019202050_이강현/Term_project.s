...
Main
	...
	LDR r1, =filter ;filter data
	LDR r2, Save_Result_Address
	MOV r3, #0 ; data index
	MOV r4, #0 ; filter index
	
loop
	CMP r3, #1920 ;480*4 if data index over 480 not multiplier
	LDRLT r5, [r0,r3]
	LDRLT r6, [r1,r4]
	BLLT multiplier
	ADD r4,r4,#4 ;filter +1
	SUB r3,r3,#4 ;data index -1
	CMP r3,#0 
	BLT negdataindex
	CMP r4, #24
	BLT loop
	
negdataindex ;when dataindex has minus
	ADD r3,r3,r4 ;data index initialize +1
	MOV r4,#0 ;filter index initialize
	B endloop
	
endloop
	MOV r9,#1920 ; 1940 isn't made with rotate
	ADD r9,#20
	BL Result
	ADD r3,r3,#4
	MOV r13,#0
	...
	LDR r4, Save_Result_Address
	SUB r6, r2,r4 ; r6 is data amount *4
	LSR r6,#1 ; because data is 4byte unit save but index is 2byte unit save
	ADD r7, r2,r6 ; r7 is last address condition where the index should be stored
	ADD r4,r4,#2000 ; r4 = index store address
	
indexalign ; data and index link
	LDR r5, [r4], #4 ; r5 = index 
	CMP r2,r7
	STRHLT r5, [r2], #2
	BLT indexalign
...
Result ; Store less that -4.7 of the convolution results(32bit, double word) and their indices(word).
	...
	ADD r8,#0x00960000
	ADD r8,#0x00006600
	ADD r8,#0x00000066 ; -4.7 ~= 0xC0966666 isn't made with rotate
	
	MOV r5,r13,LSR #31
	CMP r5,#0
	MOVEQ pc,lr  ;result is positive
	MOV r5,r13,LSL #1
	MOV r6,r8, LSL #1
	...
	MOV r10,r13,LSL #9
	MOV r11,r8,LSL #9
	MOV r10,r10, LSR #9 ; mantissa 
	MOV r11,r11,LSR #9 ; mantissa 
	
	CMP r5,r6 ; exponent compare
	MOVLT pc,lr
	CMPEQ r10,r11 ; mantissa compare
	
	STRGT r13,[r2]
	LSRGT r3, #2
	STRGT r3,[r2,#2000] ;index is stored after 2000 from data
	LSLGT r3, #2
	ADDGT r2,r2,#4
...
multiplier
	...
	MOV r7,r5,LSR #31 ; to store signbit
	MOV r8,r6,LSR #31
	
	MOV r9,r5,LSR #23 ; to store exponent
	MOV r10,r6,LSR #23
	MOV r9,r9,LSL #24 
	MOV r10,r10,LSL #24 
	MOV r9,r9,LSR #24
 	MOV r10,r10,LSR #24 ; to only store exponent not signbit
	
	MOV r11,r5,LSL #9 ; to store mantissa
	MOV r12,r6,LSL #9
	MOV r11,r11,LSR #9
	MOV r12,r12,LSR #9 ; to add 1
	
	ADD r11,r11,#0x00800000 ; add 1
	ADD r12,r12,#0x00800000
	
	CMP r7,r8
	
	MOVEQ r7,#0x00000000 ;signbit 0
	MOVNE r7,#0x80000000 ;signbit 1
	
	ADD r9,r9,r10
	SUB r9,#127; exponent calculation
...
adder
	...
	MOV r5,r6,LSR #31; to store signbit
	MOV r7,r13,LSR #31
	
	MOV r8,r6,LSR #23; to store exponent
	MOV r9,r13,LSR #23
	MOV r8,r8,LSL #24 
	MOV r9,r9,LSL #24 
	MOV r8,r8,LSR #1
 	MOV r9,r9,LSR #1 ; to only store exponent not signbit
	
	MOV r10,r6,LSL #9 ; to store mantissa
	MOV r11,r13,LSL #9
	MOV r10,r10,LSR #9
	MOV r11,r11,LSR #9 ; to add 1
	
	ADD r10,r10,#0x00800000 ; add 1
	ADD r11,r11,#0x00800000
	
	CMP r8,r9; exponent compare
	
	SUBGT r6,r8,r9
	SUBLT r6,r9,r8
	MOVEQ r6,#0
	
	MOV r6,r6, LSR #23 ; r6 is shiftnum
	
	MOVGT r11,r11,LSR r6 ; exponent matching
	MOVLT r10,r10,LSR r6
	
	MOVLT r8,r9 ; large exponent is standard
...
notequal
	...
	MOVLT r5,r7 ; large mantissa' absolute value is standard
	B normalization
	
normalization ; bit position align
	CMP r13,#0x01000000 ; mantissa shift right
	
	ADDGE r8,r8,#0x00800000
	MOVGE r13,r13,LSR #1
	SUBGE r13,r13,#0x00800000 ;sub 1 from 1.mantissa
	BGE endline
	
	CMP r13,#0x00800000 ;mantissa shift left
	
	SUBLT r8,r8,#0x00800000
	MOVLT r13,r13,LSL #1
	BLT normalization
	SUB r13,r13,#0x00800000 ;sub 1 from 1.mantissa
	B endline
...
filter DCD 0x3FF9999A, 0x3FDC28F6, 0xBEDCAC08, 0xBFA39581, 0xBF4D5CFB, 0xBE589375
Sampled_Data
