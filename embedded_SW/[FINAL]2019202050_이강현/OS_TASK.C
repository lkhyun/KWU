...
typedef struct {
    ...
    char detail[50];
    char success[50];
}TaskHistory; // Save request history struct
TaskHistory task_history[100]; //Save request history
int history_index = 0; // Request record count
/*
*********************************************************************************************************
...
INT8U MyKernelFunc(INT8U prio, char *buffer)
{
    char request[50];
    char temp[100];
    switch (prio) {
    ...
    case 14:
        ...
        break;
    default:
        strcpy(request, "unknown"); // Handle unexpected cases
        break;
    }//string conversion
    task_history[history_index].task_prio = prio;// Save priority
    strcpy(task_history[history_index].task_name,request);// Save request
    strcpy(task_history[history_index].detail, temp);//Save request detail
    OSTaskResume(prio);//Execute tasks corresponding to priority
    return (OS_NO_ERR);
}
INT8U MyKernelPrintFunc()
{
    printf("Task Execution History:\n");
    for (int i = 0; i < history_index; i++) {
        printf("Task Priority: %u, Request: %s %s %s\n", task_history[i].task_prio, task_history[i].task_name, task_history[i].detail,task_history[i].success);
        
    }//Print all request
    return (OS_NO_ERR);
}
INT8U MyKernelResponseFunc(INT8U response)
{
    if (response == 1)//success
    {
        strcpy(task_history[history_index].success,"Success");
    }
    else if(response == 0)//Failure
    {
        strcpy(task_history[history_index].success, "Failure");
    }
    else//No need detail
    {
        strcpy(task_history[history_index].success, " ");
    }
    history_index = (history_index + 1) % 100; //circular
    return (OS_NO_ERR);
}
#endif
/*$PAGE*/
