////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// File Name   :2019202050_preforked_server.c                                                                     //
// Date      :2023/05/14                                                                                          //
...
// Description   :  This code implements forking child processes in advance and assigning child processes to the  //
// client according to the client's request to work. Each process keeps a record of the clients it connects to.   //
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static int maxNchildren; //forked child numbers
static pid_t *pids;//child's pids
...
void signalhandler(int signal) // Works when signal comes in
{
   ...
   if (signal == SIGALRM) // When the set alarm time runs out
   {
      ...
      for(int i=0;i<5;i++)
      {
         kill(pids[i],SIGUSR1);//Signals children to print connection histories
      }
      alarm(10); // Snooze in 10 seconds
   }
   else if (signal == SIGINT)//input ctrl + c
   {
      for (int i = 0; i < 5; i++)
      {
         kill(pids[i], SIGTERM);//Order to quit children
         childpid = wait(&status);//wait for children to exit
   ...
   else if (signal == SIGUSR1)
   {
      ...
      {
         printf("%d\t%s\t%d\t%d\t%s", i, inet_ntoa(ADDR_history[num].sin_addr), getpid(), ADDR_history[num].sin_port, ctime(&TIME_history[num]));//print history
      }
   }
   else if (signal == SIGTERM)//parent's exit signal
   {
      exit(0);//end
   }
}
...
void child_main(int p, int socket_fd, int addrlen)
{
   ...
   signal(SIGUSR1,signalhandler);//parent's print signal
   ...
   printf("[%s] %d process is forked.\n", timetmp, getpid());//process creation message
...
int main(int argc, char **argv) // argc: parameter number, argv[]: parameter
{
   ...
   int socket_fd,addrlen;                  // socket descriptor
   int opt = 1;
   ...
   {
      pids[i] = child_make(i, socket_fd,addrlen);//Create 5 children
   }
